...
sub Sync {
            ...
            if ( !%UserData ) {
                ...
                if ( !$UserID ) {
                    ...
                    my $UserSyncInitialGroups = $Self->{ConfigObject}->Get(
                        'AuthSyncModule::AD::UserSyncInitialGroups' . $Self->{Count}
                    );
    ...
    my $UserSyncGroupsDefinition = $Self->{ConfigObject}->Get(
        'AuthSyncModule::AD::UserSyncGroupsDefinition' . $Self->{Count}
    );
    ...
    if ($UserSyncRolesDefinition) {
        ...
        for my $GroupDN ( sort keys %{$UserSyncRolesDefinition} ) {
                ...
                $Self->{LogObject}->Log(
                    Priority => 'error',
                    Message  => "Search failed! ($GroupDN) filter='$UserDN' " . $Result->error,
                );

            # extract it
	    my $Valid = _IsMemberOf($LDAP, $UserDN, $GroupDN);

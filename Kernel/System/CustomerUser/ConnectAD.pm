# --
# Kernel/System/CustomerUser/ConnectAD.pm - some customer user functions for Active directory (based on LDAP.pm), 
...
package Kernel::System::CustomerUser::ConnectAD;
...
sub new {
    ...
    # Net::LDAP new params
    elsif ( $Self->{ConfigObject}->Get( 'AuthModule::ConnectAD::Params' . $Param{Count} ) ) {
    ...
    # charset settings
    $Self->{SourceCharset} = $Self->{CustomerUserMap}->{Params}->{SourceCharset} || 'utf-8';
    $Self->{DestCharset}   = $Self->{CustomerUserMap}->{Params}->{DestCharset}   || 'utf-8';
